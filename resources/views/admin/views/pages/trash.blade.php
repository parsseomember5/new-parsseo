...
@section('content')
    <h4 class="py-3 breadcrumb-wrapper mb-4">
        <span class="text-muted fw-light">برگه‌ها /</span> زباله‌دان
    </h4>
    ...
    <div class="card">
        <div class="card-header d-flex flex">
            <div class="d-flex align-items-center">
                <form action="{{route('pages.search.trash')}}">
                ...
                @if(isset($query))
                    <a href="{{route('pages.trash')}}" class="btn btn-sm btn-secondary ms-3"><i class="bx bx-x"></i></a>
                @endif
            </div>
            <div class="ms-auto text-end primary-font pt-3 pt-md-0">
                <a href="{{route('pages.index')}}" class="btn btn-primary"><span><i class="bx bx-arrow-from-left me-sm-2"></i> <span class="d-none d-sm-inline-block">بازگشت به برگه‌ها</span></span></a>
                <form action="{{route('pages.trash.empty')}}" method="post" class="d-inline-block">
                    @csrf
                ...
                <table class="table table-striped table-hover" style="min-height: 200px">
                    ...
                    @foreach($pages as $page)
                        <tr>
                            ...
                            <td>
                                <form action="{{route('pages.delete.force')}}" method="post" class="d-inline-block">
                                    @csrf
                                    <input type="hidden" name="id" value="{{$page->id}}">
                                    <span class="btn btn-danger btn-sm btn-fore-delete" data-alert-message="این برگه برای همیشه پاک خواهد شد!">حذف</span>
                                </form>
                                <form action="{{route('pages.restore',$page->id)}}" method="post" class="d-inline-block">
                                    @csrf
