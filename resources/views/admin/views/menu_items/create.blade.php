...
@section('content')
    <h4 class="py-3 breadcrumb-wrapper mb-4">
        <span class="text-muted fw-light">آیتم های منو /</span> افزودن جدید
    </h4>

    @include('admin.includes.alerts',['class' => 'mb-3'])
    <div class="card mb-4">
        <div class="card-body">
            <form action="{{route('menu-items.store')}}" method="post" enctype="multipart/form-data" class="row">
                @csrf
                ...
                <div class="mb-3 col-lg-3">
                    ...
                    <input type="text" class="form-control" dir="ltr" id="link" name="link" value="{{old('link')}}">
                </div>
                <div class="mb-3 col-lg-3">
                    <label class="form-label" for="menu_id">منو</label>
                    <select class="form-select" name="menu_id" id="menu_id">
                        @foreach(\App\Models\Menu::all() as $menu)
                            <?php if ($loop->first) $firstMenuId = $menu->id;?>
                ...
                <div class="mb-3 col-lg-3">
                    ...
                    <select class="form-select" name="parent_id" id="parent_id">
                        ...
                        @if(isset($firstMenuId))
                        @foreach(\App\Models\MenuItem::where('menu_id',$firstMenuId) ->get() as $menuItem)
                            <option value="{{$menuItem->id}}" {{old('parent_id') == $menuItem->id ? 'selected' : ''}}>{{$menuItem->title}}</option>
                        @endforeach
                        @endif
                    </select>
                </div>
